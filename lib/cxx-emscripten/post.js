...
var encodeGray = Module.encodeGray = function(pix, width, height, stride, quality) {
...
var encodeRGB = Module.encodeRGB = function(pix, width, height, stride, quality) {
	var s = guetzli_encode_RGB(pix, width, height, stride, quality)
	var q = guetzli_cToUint8Array(guetzli_string_data(s), guetzli_string_size(s))
	guetzli_string_delete(s)
	return q
}
var encodeRGBA = Module.encodeRGBA = function(pix, width, height, stride, quality) {
...
var encodeImage = Module.encodeImage = function(pix, width, height, channels, stride, quality) {
	if(channels != 1 && channels != 3 && channels == 4) {
		throw "emscripten: encodeImage, invalid channels = " + (channels|0)
	}
	switch(channels) {
	case 1: return encodeGray(pix, width, height, stride, quality)
	case 3: return encodeRGB(pix, width, height, stride, quality)
	case 4: return encodeRGBA(pix, width, height, stride, quality)
	}
	throw "unreachable!"
}
